...
```
Liste des variable avec un taux na > 50%
```{r}
var_to_valid <- descriptor$var_name[as.numeric(as.character(descriptor$ratio_na))>0.5] 
```


```{r}
str(dt_[var_to_valid])
summary(dt_[var_to_valid])
```
