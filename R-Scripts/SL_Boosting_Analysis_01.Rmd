...
k_cv_sampling = 10
model_type = "adaboost"
train_results_name = "SL_Adaboost_01"
```
...
train_results = list()
gridsearch <- expand.grid(nIter=seq(1, 100, 5),method=c("adaboost"))
...
prediction <- predict(object=train_results[[samp_]]$finalModel, dta_.test.x, type='class')
head(prediction)
conf_matrix <- confusionMatrix(prediction$class, dta_.test.y)
conf_matrix
cm.plot(conf_matrix$table)
#plot(varImp(train_results[[samp_]]$finalModel), main="Variable Importance")
