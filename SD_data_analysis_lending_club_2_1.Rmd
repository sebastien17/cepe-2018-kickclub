...
```{r}
# Reprise de la fonction de Séb sur les dates pour convertir les dates format 'factor' en dates format 'Date'
...
period_factor2 <- function(x){
  ...
  return(x.date) # à la place de return(as.factor(x.date))
...
- "issue_y" l'année plutôt que le mois ("issue_d") d'émission du pret
...
Une trop forte corrélation entre les variables fait parfois planté certains modèles
...
Je proposerais la liste suivante des variables à conserver:
...
Les données mnths_xxxx posent problème
* mths_since_last_delinq: 0
* mths_since_last_record: 0
* mths_since_last_major_derog: 0
Comme observé précédemment, nous devrions traiter les NAs à l'opposé: un nombre de mois élévé depuis la dernière infraction
Or, c'est impossible de déterminer 
On pourrait:
- soit les convertir en 'factor' (avec un 'level' 'inconnu' pour les NAs) 
- soit les éliminer tous les trois
Pour faire simple et rapide, je suggère de les éliminer pour l'instant.
